...
function StoneBombs:BombInit(bomb)
	...
	if player then
		...
		if player:HasCollectible(RestoredItemsPack.Enums.CollectibleType.COLLECTIBLE_STONE_BOMBS) then
			if (bomb.Variant > BombVariant.BOMB_SUPERTROLL or bomb.Variant < BombVariant.BOMB_TROLL) then
				if bomb.Variant == 0 then
					bomb.Variant = RestoredItemsPack.Enums.BombVariant.BOMB_STONE
				end
			end
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_BOMB_INIT, StoneBombs.BombInit)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_BOMB_UPDATE, StoneBombs.BombUpdate)
