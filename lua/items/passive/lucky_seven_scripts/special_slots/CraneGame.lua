...
local function ShootGrappleAtClosestEnemy(slot, data)
    ...
    local cordEnd = Isaac.Spawn(EntityType.ENTITY_EFFECT, RestoredItemsPack.Enums.Entities.LUCKY_SEVEN_CORD_END.Variant, 0, slot.Position, targetClawVelocity, slot):ToEffect()
    cordEnd.Parent = slot
    cordEnd:Update()
...
local function SpawnCord(cordEnd)
    local head = Isaac.Spawn(EntityType.ENTITY_EFFECT, RestoredItemsPack.Enums.Entities.LUCKY_SEVEN_CORD_HANDLER.Variant, 0, cordEnd.Position, Vector.Zero, cordEnd):ToEffect()
    ...
    local rope = Isaac.Spawn(EntityType.ENTITY_EVIS, 10, RestoredItemsPack.Enums.Entities.LUCKY_SEVEN_CRANE_CORD.SubType, cordEnd.Parent.Position, Vector.Zero, cordEnd)
    cordEnd.Child = rope
    head.Child = rope
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, OnCordEndUpdate, RestoredItemsPack.Enums.Entities.LUCKY_SEVEN_CORD_END.Variant)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, OnCordHandlerUpdate, RestoredItemsPack.Enums.Entities.LUCKY_SEVEN_CORD_HANDLER.Variant)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_PRE_NPC_UPDATE, PreHorfHandlerUpdate, EntityType.ENTITY_HORF)


local function PreEvisCordUpdate(_, npc)
    if npc.Variant == 10 and npc.SubType == RestoredItemsPack.Enums.Entities.LUCKY_SEVEN_CRANE_CORD.SubType then
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_PRE_NPC_UPDATE, PreEvisCordUpdate, EntityType.ENTITY_EVIS)
