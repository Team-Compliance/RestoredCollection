...
function SafetyBombsMod:BombInit(bomb)
	if Helpers.GetData(bomb).BombInit or bomb.Variant == BombVariant.BOMB_ROCKET then return end
	local player = Helpers.GetPlayerFromTear(bomb)
	if player then
		local rng = bomb:GetDropRNG()
		local safeChance = player:HasCollectible(RestoredCollection.Enums.CollectibleType.COLLECTIBLE_SAFETY_BOMBS) and 
        (not bomb.IsFetus or bomb.IsFetus and rng:RandomInt(100) < 20)

		local nancyChacne = player:HasCollectible(CollectibleType.COLLECTIBLE_NANCY_BOMBS) and
