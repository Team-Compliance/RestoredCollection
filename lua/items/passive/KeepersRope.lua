...
local function GetRope(player,bool)
	bool = bool or false
	for _,rope in ipairs(Isaac.FindByType(EntityType.ENTITY_EFFECT, RestoredItemsPack.Enums.Entities.KEEPERS_ROPE.Variant)) do
		rope = rope:ToEffect()
...
function KeepersRope:Rope(player)
	...
	local hasMorphedKeepersRope = TSIL.SaveManager.GetPersistentVariable(RestoredItemsPack, "HasMorphedKeepersRope")
	if not hasMorphedKeepersRope then
		for _, _ in ipairs(Isaac.FindByType(5,100,RestoredItemsPack.Enums.CollectibleType.COLLECTIBLE_KEEPERS_ROPE)) do
			TSIL.SaveManager.SetPersistentVariable(RestoredItemsPack, "HasMorphedKeepersRope", true)
		end
	end
	if player:HasCollectible(RestoredItemsPack.Enums.CollectibleType.COLLECTIBLE_KEEPERS_ROPE) then
		TSIL.SaveManager.SetPersistentVariable(RestoredItemsPack, "HasMorphedKeepersRope", true)
		if not BeastFight and not player:IsDead() then
			if not GetRope(player, true) then
				local rope = Isaac.Spawn(EntityType.ENTITY_EFFECT, RestoredItemsPack.Enums.Entities.KEEPERS_ROPE.Variant, 0, player.Position, Vector.Zero, player):ToEffect()
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_PLAYER_UPDATE, KeepersRope.Rope)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_EFFECT_INIT, KeepersRope.RopeInit, RestoredItemsPack.Enums.Entities.KEEPERS_ROPE.Variant)
...
function KeepersRope:RopeUpdate(rope)
	...
	--rope.Position = player.Position
	if player:HasCollectible(RestoredItemsPack.Enums.CollectibleType.COLLECTIBLE_KEEPERS_ROPE) == false or player:IsDead() then
		rope:Remove()
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, KeepersRope.RopeUpdate, RestoredItemsPack.Enums.Entities.KEEPERS_ROPE.Variant)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_NPC_INIT, KeepersRope.HereComesTheMoney)
		...
		if entity:IsVulnerableEnemy() and data.CoinsToBeat then			
			if data.CoinsToBeat > 0 then
				for _, player in ipairs(Helpers.GetPlayers()) do
					...
					local ropeRNG = player:GetCollectibleRNG(RestoredItemsPack.Enums.CollectibleType.COLLECTIBLE_KEEPERS_ROPE)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, KeepersRope.MoneyMoneyMoneyMoney)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_NPC_RENDER, KeepersRope.MoneyMoneyMoneyMoneyMoney)
...
function KeepersRope:DollarDollar(npc)
	...
	if data.CoinsToBeat and data.CoinsToBeat > 0 then
		for _ = 1, data.CoinsToBeat do
			for _, player in ipairs(Helpers.GetPlayers()) do
				local ropeRNG = player:GetCollectibleRNG(RestoredItemsPack.Enums.CollectibleType.COLLECTIBLE_KEEPERS_ROPE):RandomInt(3)
				local pickup
				local vector = Vector(math.random(1,3) * (math.random(1,2) == 1 and 1 or -1), math.random(1,3) * (math.random(1,2) == 1 and 1 or -1))
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_ENTITY_KILL, KeepersRope.DollarDollar)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_EVALUATE_CACHE, KeepersRope.NoSoap)


function KeepersRope:RopeReplacement(keeper)
	if not TSIL.SaveManager.GetPersistentVariable(RestoredItemsPack, "HasMorphedKeepersRope") then
		for __,pickup in ipairs(Isaac.FindByType(5,100)) do
			...
			or pickup.SubType == CollectibleType.COLLECTIBLE_COUPON) then
				...
				if pickData.RNG:RandomInt(3) == 0 and pickup.SubType ~= RestoredItemsPack.Enums.CollectibleType.COLLECTIBLE_KEEPERS_ROPE then
					pickup:Morph(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_COLLECTIBLE, RestoredItemsPack.Enums.CollectibleType.COLLECTIBLE_KEEPERS_ROPE, true, true, false)
					break
				end
			end
		end
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_NPC_DEATH, KeepersRope.RopeReplacement, EntityType.ENTITY_SHOPKEEPER)
