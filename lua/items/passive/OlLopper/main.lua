...
OlLopper.HEAD_FLYING_OFFSET = Vector(0, -7)
...
function RenderPlayerNeck(parent, headPos)
    ...
    local playerPos = player.Position + Vector(0, -17) + player.PositionOffset
    local neckPos = headPos + Vector(0, -17) + (player.CanFly and OlLopper.HEAD_FLYING_OFFSET or Vector.Zero)
    ...
    for i = 1, segments-1, 1 do
        ...
        local targetPos = playerPos + direction:Resized(i * distanceStep) + (player.CanFly and OlLopper.HEAD_FLYING_OFFSET or Vector.Zero)
        ...
        neckPiece.DepthOffset = neckPiece.Position:Distance(parent.Position) - neckPiece.Position:Distance(player.Position) - i
    ...
    function OlLopper:PrePlayerRenderReflection(player, renderPos)
        if Game():GetRoom():GetRenderMode() == RenderMode.RENDER_WATER_REFLECT then
            ...
            if headHelper and GetPtrHash(headHelper:ToFamiliar().Player) == GetPtrHash(player) then
                ...
                player:RenderBody(Isaac.WorldToRenderPosition(player.Position) + renderPos)
                return false
            end
        end
    end
    RestoredCollection:AddCallback(ModCallbacks.MC_PRE_PLAYER_RENDER, OlLopper.PrePlayerRenderReflection)
else
