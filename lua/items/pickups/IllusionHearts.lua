...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_PLAYER_UPDATE, IllusionModLocal.UpdateClones)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, IllusionModLocal.CloneRoomUpdate)
...
function IllusionMod:addIllusion(player, isIllusion, addWisp)
	...
	if addWisp then
		local wisp = player:AddWisp(RestoredItemsPack.Enums.CollectibleType.COLLECTIBLE_BOOK_OF_ILLUSIONS, player.Position)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_PEFFECT_UPDATE, IllusionModLocal.CloneCache)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_EVALUATE_CACHE, IllusionModLocal.HackyLazWorkAround)
...
function IllusionModLocal:preIllusionHeartPickup(pickup, collider)
	...
	if player then
		...
		if pickup.Variant == PickupVariant.PICKUP_HEART and pickup.SubType == RestoredItemsPack.Enums.Pickups.Hearts.HEART_ILLUSION and not player.Parent then
			pickup.Velocity = Vector.Zero
			pickup.EntityCollisionClass = EntityCollisionClass.ENTCOLL_NONE
			pickup:GetSprite():Play("Collect", true)
			pickup:Die()
			IllusionMod:addIllusion(player, true, false)
			sfx:Play(RestoredItemsPack.Enums.SFX.Hearts.ILLUSION_PICKUP, 1, 0, false)
			return true
		end
	end
end
RestoredItemsPack:AddCallback(ModCallbacks.MC_PRE_PICKUP_COLLISION, IllusionModLocal.preIllusionHeartPickup)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_PRE_PLAYER_COLLISION, IllusionModLocal.preIllusionWhiteFlame)

---@param pickup EntityPickup
function IllusionModLocal:PreGoldenSpawn(pickup)
	if TSIL.Random.GetRandom(pickup.InitSeed) >= (1 - TSIL.SaveManager.GetPersistentVariable(RestoredItemsPack, "IllusionHeartSpawnChance") / 100) 
	and pickup.SubType == HeartSubType.HEART_GOLDEN then
		pickup:Morph(pickup.Type, PickupVariant.PICKUP_HEART, RestoredItemsPack.Enums.Pickups.Hearts.HEART_ILLUSION, true, true)
	end
end
RestoredItemsPack:AddCallback(TSIL.Enums.CustomCallback.POST_PICKUP_INIT_FIRST, IllusionModLocal.PreGoldenSpawn, PickupVariant.PICKUP_HEART)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, IllusionModLocal.onEntityTakeDamage, EntityType.ENTITY_PLAYER)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_ENTITY_REMOVE, IllusionModLocal.AfterDeath)
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_POST_NPC_RENDER, IllusionModLocal.DarkEsau, EntityType.ENTITY_DARK_ESAU)

function IllusionModLocal:ClonesControls(entity,hook,action)
	if entity ~= nil and entity.Type == EntityType.ENTITY_PLAYER and not TSIL.SaveManager.GetPersistentVariable(RestoredItemsPack, "IllusionClonesPlaceBombs") then
...
RestoredItemsPack:AddCallback(ModCallbacks.MC_INPUT_ACTION, IllusionModLocal.ClonesControls)