...
function VoodooPin:VoodooThrow(player)
	...
	if state and data.VoodooWaitFrames <= 0 then
		...
		if (left > 0 or right > 0 or down > 0 or up > 0 or mouseclick) and data.VoodooWaitFrames <= 0 then
			...
			Isaac.Spawn(EntityType.ENTITY_TEAR, RestoredCollection.Enums.TearVariant.VOODOO_PIN, 0, player.Position, vecShoot, player):ToTear()
...
RestoredCollection:AddCallback(ModCallbacks.MC_PRE_TEAR_COLLISION, VoodooPin.VoodooHit, RestoredCollection.Enums.TearVariant.VOODOO_PIN)
...
RestoredCollection:AddCallback(ModCallbacks.MC_POST_TEAR_UPDATE, VoodooPin.VoodooPinThrown, RestoredCollection.Enums.TearVariant.VOODOO_PIN)


function VoodooPin:VoodooShatter(pin)
	if pin.Variant == RestoredCollection.Enums.TearVariant.VOODOO_PIN then
